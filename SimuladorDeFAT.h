...
const uint16_t StartDataClusterEntrys = 0x2800; //endereço de memória em que começa as entradas de diretório 
const uint16_t DataClusterEntrys = 0x226c0; //endereço de memória que termina as entradas de diretório
	  uint16_t freeCluster; //Aponta para o endereço de memória do cluster livre
/*--------------------------------------------------------------------------------------------*/

/*Constantes utilizadas para comparação na busca pelos diretórios-----------------------------*/
const uint16_t freeClusterValue = 0x0000; //indica que o cluster esta vazio e/ou pode ser reescrito
...
int loadFat();
int loadRootDir();
int writeOnDisk(dataCluster* buffer, uint8_t type, char directory[]); //O buffer inicial pussuí 1024bytes de tamanho
void printFat();
